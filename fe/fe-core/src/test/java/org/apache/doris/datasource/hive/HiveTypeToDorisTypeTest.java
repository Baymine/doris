...
public class HiveTypeToDorisTypeTest {
    ...
    public void testDecimalType() {
        // explicit precision/scale is preserved unchanged
        Assert.assertEquals(ScalarType.createDecimalV3Type(10, 2), hiveTypeToDorisType("decimal(10,2)", 0));
        Assert.assertEquals(ScalarType.createDecimalV3Type(18, 4), hiveTypeToDorisType("decimal(18,4)", 0));
        // bare `decimal` (no precision/scale) maps to Hive-consistent decimal(38,10),
        // NOT ScalarType.DEFAULT_PRECISION(9)/DEFAULT_SCALE(0) which silently truncates.
        Assert.assertEquals(
                ScalarType.createDecimalV3Type(
                        HiveMetaStoreClientHelper.HIVE_DEFAULT_DECIMAL_PRECISION,
                        HiveMetaStoreClientHelper.HIVE_DEFAULT_DECIMAL_SCALE),
